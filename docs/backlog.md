...
| C-07 | Built-in SoundFont (`.sf2`) synth module | todo | P2 | 2-4w | — | Verified: no MIDI module in the 4.7.1 tree; DB depends on external MidiStream GDExtension. Additive module (new files only, zero rebase risk): `.sf2` loader + synth + `AudioStream`; pairs with C-03 (3D MIDI). Kills the GDExtension dependency |
