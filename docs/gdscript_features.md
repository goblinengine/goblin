...
- NOT implemented — spec locked (2026-08-19): **`@schema`** — a `const` Dictionary annotated `@schema` becomes a project-wide reusable schema; `Dictionary[Name]` instantiates it (defaults autofilled at construction, typed override-merge, growable beyond the schema). The reference title's `Data` system (`data.gd`) implements templates as a centralized, registered three-level runtime fallback chain (`instance → Objects.list[id] → _types[type].defaults`) keyed by `id`/`type` StringNames. Engine surface per RFC §2.0 (reuse const-as-type in `resolve_datatype` + G-17 shape fields on `GDScriptDataType`/`DataType`, defaults vector serialized in `append_datatype`/`decode_datatype`, filled in `OPCODE_CONSTRUCT_SHAPED_DICTIONARY`, global schema registry; no `core/variant` change). TD-04 gates future `Dictionary` runtime-perf hardening under real fallback usage.
...
See [backlog.md](backlog.md) §1. Next priorities: `then`/`elthen` tests (TD-02), structs (G-07), typed dictionaries (G-08). Template dictionaries (G-18) — design discussion, no syntax decided (see §Shaped Dictionary Literals + RFC §2).
