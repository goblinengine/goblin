...
- NOT implemented (rejected at engine level, 2026-08-19): template dictionaries live GDScript-side. The reference title's `Data` system (`data.gd`) implements templates as a centralized, registered three-level runtime fallback chain (`instance → Objects.list[id] → _types[type].defaults`) keyed by `id`/`type` StringNames — no per-dict `_template` metadata. Putting template semantics in `core/variant` `Dictionary` would duplicate that registry in C++ (header-blast-radius + ODR/ABI hazard, rules B-09) and the reserved-`_template`-key shape leaks into `keys()`/`merge()`/`size()`/`hash()`/`==` on every dictionary. Fork involvement is limited to `Dictionary` runtime-perf hardening under real fallback usage, gated by TD-04 (backlog). See G-18 (rejected).
...
See [backlog.md](backlog.md) §1. Next priorities: `then`/`elthen` tests (TD-02), structs (G-07), typed dictionaries (G-08). Template dictionaries (G-18) rejected at engine level — see §Shaped Dictionary Literals (templates live GDScript-side).
