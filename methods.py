...
def detect_modules(search_path, recursive=False):
    ...
    def is_engine(path):
        ...
        # Goblin sources when using `custom_modules` build option.
        version_path = os.path.join(path, "version.py")
        if os.path.exists(version_path):
            with open(version_path) as f:
                if 'short_name = "goblin"' in f.read():
                    return True
        return False
...
def detect_visual_c_compiler_version(tools_env):
    ...
    # There are other architectures, but Goblin does not support them currently, so this function does not detect arm/amd64_arm
...
def generate_vs_project(env, num_jobs):
    ...
    if batch_file:
        ...
        debug_targets = ["bin\\goblin.windows.tools.32.exe"] + ["bin\\goblin.windows.tools.64.exe"]
        release_targets = ["bin\\goblin.windows.opt.32.exe"] + ["bin\\goblin.windows.opt.64.exe"]
        release_debug_targets = ["bin\\goblin.windows.opt.tools.32.exe"] + ["bin\\goblin.windows.opt.tools.64.exe"]
