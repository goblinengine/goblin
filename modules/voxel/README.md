...
Assign a VoxelLevelGenerator to the `World`'s `Level Generator` property.

You can write your own algorithm by implementing the ``` void _generate_chunk(chunk: VoxelChunk) virtual ``` method.

`VoxelLevelGeneratorFlat` is also available, it will generate a floor for you, if you use it.
...
```git clone https://github.com/Relintai/voxel.git voxel```

(the folder needs to be named voxel!)
